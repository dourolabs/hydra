...
#[tokio::test]
async fn s3_storage_client_round_trip_with_hydra_s3() -> Result<(), BuildCacheError> {
    ...
    wait_for_hydra_s3(&client).await?;
...
async fn wait_for_hydra_s3(client: &S3StorageClient) -> Result<(), BuildCacheError> {
...
async fn s3_error_includes_http_status_for_missing_object() {
    ...
    wait_for_hydra_s3(&client).await.expect("server ready");
