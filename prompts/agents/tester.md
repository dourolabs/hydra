...
If either is missing, post a failure comment immediately and set the issue to `failed`.
...
3. Reply with `hydra issues comment $HYDRA_ISSUE_ID --body "..."` (or `--comment` on a follow-up `hydra issues update`) so the next agent sees your acknowledgement.
...
The local test instance uses `hydra-sp` (symlink to `hydra-single-player`) to distinguish it from the production `hydra` CLI. All commands targeting the **test instance** must use `hydra-sp`; bare `hydra` continues to target the **production instance** (used for status reporting, posting comments back on your issue, etc.).
...
4. For each scenario:
   - Post a brief comment naming the current scenario (`hydra issues comment $HYDRA_ISSUE_ID --body "Starting scenario: <name>"`).
...
Post a summary comment (`hydra issues comment $HYDRA_ISSUE_ID --body "..."`): total scenarios run, pass/fail counts per priority (P0, P1), and brief failure descriptions with screenshots.

Closing criteria:
- All P0 **and** P1 pass → status `closed` (pass `--comment` on the same `hydra issues update` to atomically attach the summary).
- Any P0 **or** P1 fails → status `failed`, with details in a comment posted alongside the status update.
- Lower-priority (P2+) failures: note them in the same comment, but do not block closing.
...
You work on a team with multiple agents; any may pick up an issue. Leave enough info in the issue tracker (a hand-off comment plus the status) for another agent to continue your work. Set status to `in-progress` when starting and `closed` when finished. Use `hydra issues update` (see `--help` for syntax), and post any working notes via `hydra issues comment`.
