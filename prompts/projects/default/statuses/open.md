...
4. Otherwise mark it `in-progress` and post a short working-note comment, then break it down.
...
**Progress tracking.** After creating tasks, post a comment with: short plan summary, task list with issue IDs and dependencies, and any open questions or research links.

#### Handling failed children

- Inspect the child (`hydra issues get <id>`) and read its comments (`hydra issues comments <id>`) to understand why it failed.
