...
4. Create an issue (assigned to **swe**) to produce a docker image for the repository. The goal is to produce a PR adding "Dockerfile.hydra" to the repository
   ...
   - **No Docker build caching:** Set `no-cache: true` on the `docker/build-push-action` step.
     Docker layer caching doesn't play well with the `/latest/` release URL used for the hydra CLI
   ...
   - **PR validation trigger:** In addition to `workflow_dispatch`, add a `pull_request` trigger
     filtered to `Dockerfile.hydra` path changes. This lets PRs that modify the Dockerfile validate
   ...
   **The Dockerfile.hydra replaces the default hydra-worker image for this repo.** It must include everything
   ...
   The hydra worker executes jobs by invoking `claude` (Claude Code CLI) or `codex` (OpenAI Codex CLI) inside
   ...
   ### Required: hydra CLI binary

   **The Dockerfile must install the `hydra` CLI binary so it is available on the agent's PATH.** Download the
   pre-built binary from the hydra-releases repository using the `latest` release tag. The hydra-releases repo
   maintains a GitHub release tagged `latest` that always points to the current version.

   ```dockerfile
   # Install the hydra CLI binary (always pulls the current release)
   RUN curl -fsSL https://github.com/dourolabs/hydra-releases/releases/download/latest/hydra-x86_64-unknown-linux-gnu \
   ...
   **Do NOT hardcode a specific version tag** (e.g., `v0.4.28`) — this causes images to become stale when the
   hydra CLI is updated. Using the `latest` tag ensures every image rebuild picks up the current version.

   This ensures the `hydra` command is accessible from the CLI inside the container. The binary must be placed in a directory
   that is on the default PATH (e.g. `/usr/local/bin/`). Verify by adding a `RUN hydra --version` step in the Dockerfile
   or by documenting that the agent should test `hydra --version` after container startup.
   ...
   - **`puppeteer`** — headless browser for web fetching (install via `npm install -g puppeteer`; requires
     browser system dependencies — see the hydra-worker Dockerfile for the full list of `lib*` packages)
   ...
   This matches the hydra-worker convention. The user needs a proper home directory for npm/nvm/cargo config.

   **The Dockerfile must also set WORKDIR to a clean, empty directory** that the worker user can write to.
   The hydra agent clones repositories into the container's current working directory (`.`). If the working
   ...
   **Do NOT add an ENTRYPOINT directive or worker-entrypoint.sh script.** As of PR #1542 (i-bxcvkunct),
   the hydra server specifies the container command/args directly in the K8s job spec. Custom worker
   ...
   The pre-built hydra binary requires GLIBC 2.38 or newer. Common Debian-based images and
   ...
   When choosing a base image for Dockerfile.hydra, ensure it provides GLIBC >= 2.38. If the project needs an older
...
6. Create an issue as a follow up to (5) to update the hydra repo with the new image name. "hydra repos update <repo-name> --default-image <image-name>"
...
- **Agent jobs fail to start or produce no output for a repo with a custom image:** The repo's custom
  ...
  `hydra repos update <repo-name> --clear-default-image` so jobs fall back to the default hydra-worker
  image. Then create a new task to fix the Dockerfile.hydra. Once the fixed image is rebuilt, set
  the default-image again with `hydra repos update <repo-name> --default-image <image-name>`.
