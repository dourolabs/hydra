 ...
 * Within each topological tier, items are sorted by unread first, then by
 * lastUpdated descending. Cycles are handled gracefully by appending
 * remaining nodes in lastUpdated order. Non-issue items are appended at the
 * end sorted by unread/lastUpdated.
 */
export function topologicalSortWorkItems(
  items: WorkItem[],
  isUnread: (item: WorkItem) => boolean,
): WorkItem[] {
  ...
  if (issueItems.length === 0) {
    return [...nonIssueItems].sort((a, b) => tierCompare(a, b, isUnread));
  }
  ...
  const compareFn = (aId: string, bId: string): number =>
    tierCompare(itemMap.get(aId)!, itemMap.get(bId)!, isUnread);
  ...
  if (result.length < issueItems.length) {
    ...
    const remaining = issueItems
      ...
      .sort((a, b) => tierCompare(a, b, isUnread));
  ...
  if (nonIssueItems.length > 0) {
    result.push(
      ...nonIssueItems.sort((a, b) => tierCompare(a, b, isUnread)),
    );
  }
