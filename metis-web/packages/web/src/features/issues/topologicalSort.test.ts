...
describe("topologicalSortWorkItems", () => {
  it("returns empty array for empty input", () => {
    expect(topologicalSortWorkItems([], neverUnread)).toEqual([]);
  });

  it("returns single item unchanged", () => {
    const a = makeWorkItem("a");
    expect(workItemIds(topologicalSortWorkItems([a], neverUnread))).toEqual([
      "a",
    ]);
  });
  ...
  it("sorts a linear child-of chain: C child-of B child-of A", () => {
    ...
    const result = workItemIds(
      topologicalSortWorkItems([a, b, c], neverUnread),
    );
  ...
  it("sorts blocked-on: B blocked-on A means A before B", () => {
    ...
    const result = workItemIds(
      topologicalSortWorkItems([b, a], neverUnread),
    );
  ...
  it("sorts diamond child-of dependencies: leaf first, root last", () => {
    ...
    const result = workItemIds(
      topologicalSortWorkItems([a, b, c, d], neverUnread),
    );
  ...
  it("handles mixed child-of and blocked-on dependencies", () => {
    ...
    const result = workItemIds(
      topologicalSortWorkItems([a, c, b], neverUnread),
    );
  ...
  it("ignores edges to issues not in the active set", () => {
    ...
    const result = workItemIds(
      topologicalSortWorkItems([a, b], neverUnread),
    );
  ...
  it("bubbles unread items up within their tier", () => {
    const a = makeWorkItem("a", [], "2026-01-01T01:00:00Z");
    const b = makeWorkItem("b", [], "2026-01-01T02:00:00Z");
    // b has later timestamp so normally comes first, but a is unread
    const isUnread = (item: WorkItem) => item.id === "a";
    const result = workItemIds(
      topologicalSortWorkItems([a, b], isUnread),
    );
    expect(result).toEqual(["a", "b"]);
  });
  ...
  it("places non-issue items after issue items", () => {
    ...
    const result = workItemIds(
      topologicalSortWorkItems([patch, issue], neverUnread),
    );
  ...
  it("handles only non-issue items", () => {
    ...
    const result = workItemIds(
      topologicalSortWorkItems([p1, p2], neverUnread),
    );
