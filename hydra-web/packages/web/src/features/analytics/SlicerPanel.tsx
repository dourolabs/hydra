...
export function SlicerPanel({ state, onChange }: SlicerPanelProps) {
  ...
  const handleStatusToggle = (key: string, checked: boolean) => {
    const next = checked
      ? [...state.statusKeys, key]
      : state.statusKeys.filter((s) => s !== key);
    onChange({ statusKeys: next });
  };

  const handleIssueTypeToggle = (key: IssueTypeKey, checked: boolean) => {
    const next = checked
      ? [...state.issueTypes, key]
      : state.issueTypes.filter((s) => s !== key);
    onChange({ issueTypes: next });
  };

  return (
    <aside className={styles.panel} aria-label="Slicers" data-testid="slicer-panel">
      <div className={styles.head}>Slicers</div>

      <div className={styles.field}>
        <label className={styles.label} htmlFor="slicer-project">
          Project
        </label>
        <select
          id="slicer-project"
          ...
          onChange={(e) => onChange({ projectId: e.target.value || null, statusKeys: [] })}
      ...
      <div className={styles.field}>
        <label className={styles.label}>
          Status
          {!state.projectId && (
            <span className={styles.hint}> · select a project</span>
          )}
        </label>
        <div
      ...
      <div className={styles.field}>
        <label className={styles.label} htmlFor="slicer-repo">
          Repo
        </label>
        <select
          id="slicer-repo"
