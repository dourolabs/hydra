...
export interface BreadcrumbsContextValue {
  ...
  setBreadcrumbs: (items: BreadcrumbItem[], current: string, currentKind?: "code") => void;
...
export function useBreadcrumbs(
  ...
  const key = JSON.stringify({ items, current, currentKind });
  useEffect(() => {
    setBreadcrumbs(items, current, currentKind);
