...
- `@projects:details-rail-project-block` — The issue detail right-rail Details tab includes a Project row between Status and Assignee. The row renders a `<ProjectChip>` with the issue's resolved project key + name. Issues with no `project_id` fall back to the synthesized default project.
