// This file was generated by [ts-rs](https://github.com/Aleph-Alpha/ts-rs). Do not edit this file manually.
import type { SessionId } from "./SessionId";

export type SearchDocumentsQuery = { q: string | null, path_prefix: string | null, path_is_exact: boolean | null, created_by: SessionId | null, include_deleted: boolean | null, 
/**
 * Maximum number of results to return. When omitted, all results are returned.
 */
limit: number | null, 
/**
 * Opaque cursor from a previous response's `next_cursor` field.
 */
cursor: string | null, 
/**
 * When true, include `total_count` in the response.
 */
count: boolean | null, };
