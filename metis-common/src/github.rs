...
fn resolve_per_page(config: &GithubConfig) -> Result<u8> {
    ...
    if let Ok(raw) = env::var(ENV_GITHUB_PER_PAGE) {
        let trimmed = raw.trim();
        if trimmed.is_empty() {
            return Ok(DEFAULT_GITHUB_PER_PAGE);
        }
        let parsed: u8 = trimmed
            .parse()
            .with_context(|| format!("invalid {ENV_GITHUB_PER_PAGE} value '{trimmed}'"))?;
        return validate_per_page(parsed);
    }
...
fn env_per_page() -> Option<u8> {
    let raw = env_var(ENV_GITHUB_PER_PAGE)?;
    raw.parse().ok()
}
