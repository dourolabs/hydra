...
use clap::Subcommand;
use metis_common::{
    constants::{ENV_GH_TOKEN, ENV_GITHUB_TOKEN, ENV_METIS_ID, ENV_METIS_ISSUE_ID},
    github::GithubConfig,
    issues::{
...
pub async fn resolve_service_repo_name(
    ...
    let BundleSpec::ServiceRepository { name, .. } = job.task.context else {
        return Ok(None);
    };
...
async fn open_pull_request(
    ...
    let token = github_config.resolved_token(None).ok_or_else(|| {
        anyhow!("{ENV_GH_TOKEN}/{ENV_GITHUB_TOKEN} environment variable is required")
    })?;
    let crab = github_config
