...
fn cmd_init() -> Result<()> {
    ...
    let config_content = render_server_config(&encryption_key, &github_pat, &db_path);
...
fn generate_encryption_key() -> String {
    ...
    // Use /dev/urandom via std for cryptographic randomness.
    ...
    let hash = Sha256::digest(random_seed.as_bytes());
    rng_bytes.copy_from_slice(&hash);
    use base64::Engine;
    base64::engine::general_purpose::STANDARD.encode(rng_bytes)
}

fn render_server_config(encryption_key: &str, github_pat: &str, db_path: &Path) -> String {
    format!(
...
fn start_with_launchd() -> Result<()> {
    ...
    let auth_token_path = expand_path(AUTH_TOKEN_PATH);
...
<dict>
    ...
    <dict>
        ...
        <key>METIS_AUTH_TOKEN_FILE</key>
        <string>{auth_token_path}</string>
    </dict>
    <key>RunAtLoad</key>
    <false/>
    <key>KeepAlive</key>
...
Environment=METIS_AUTH_TOKEN_FILE={auth_token_path}
Restart=on-failure
RestartSec=5
...
fn start_with_pid_file() -> Result<()> {
    ...
    let child = Command::new(&server_binary)
        ...
        .env("METIS_AUTH_TOKEN_FILE", &auth_token_path)
        .stdout(log_handle)
...
mod tests {
    ...
    fn render_server_config_contains_required_fields() {
        let config = render_server_config("dGVzdGtleQ==", "ghp_test123", Path::new("/tmp/test.db"));
