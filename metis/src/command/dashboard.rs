...
async fn run_dashboard_loop(
    ...
    let mut sse_stream = match client.subscribe_events(&EventsQuery::default(), None).await {
        Ok(stream) => {
            state.connection_state = ConnectionState::Connected;
            stream
        }
        Err(_) => {
            // Initial connection failed; enter reconnection state and retry with backoff.
            state.connection_state = ConnectionState::Reconnecting(1);
            needs_draw = true;

            let reconnect_at = Instant::now() + reconnect_delay(1);
            let quit =
            ...
            // Retry loop until we get a connection.
            let mut reconnect_attempts: u32 = 1;
            loop {
                match client
    ...
    // Main loop: run SSE, reconnect on failure.
...
fn summarize_job(job: JobSummaryRecord, now: DateTime<Utc>) -> JobDisplay {
    ...
    let runtime = output::format_runtime_fields(
        job.task.status,
        job.task.start_time,
        job.task.creation_time,
        job.task.end_time,
        now,
    );
    let last_change = job
...
mod tests {
    ...
    fn job_with_status(id: &str, status: Status, _offset_seconds: i64) -> JobSummaryRecord {
        use metis_common::jobs::{JobVersionRecord, Task};
