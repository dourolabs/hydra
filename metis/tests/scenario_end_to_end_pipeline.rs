...
use harness::{
    find_issue_summary_by_description, find_summary_children_of, merge_patch,
    test_patch_workflow_config, IssueAssertions,
};
use metis_common::{
...
async fn full_end_to_end_pipeline() -> Result<()> {
    ...
    let swe_issue = find_issue_summary_by_description(&all_issues.issues, "Implement settings UI")
    ...
    swe_issue_record.assert_has_child_with_status_in_summaries(
        &all_issues.issues,
