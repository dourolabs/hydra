...
const SERVER_CONFIG_PATH: &str = "~/.hydra/server/config.yaml";
const AUTH_TOKEN_PATH: &str = "~/.hydra/server/auth-token";
const PID_FILE_PATH: &str = "~/.hydra/server/hydra-server.pid";
const LOG_DIR: &str = "~/.hydra/server/logs";
...
pub enum ServerCommand {
    ...
    Status,
    /// Tail the local hydra server logs.
    Logs {
...
fn cmd_init(config_file: Option<PathBuf>) -> Result<()> {
    ...
    println!();
    println!("Hydra is running! Dashboard: {LOCAL_SERVER_URL}");
...
fn create_default_agents(auth_token: &str) -> Result<()> {
    ...
    for &(name, prompt, is_assignment_agent) in agents {
        let secrets = vec!["GH_TOKEN".to_string()];
...
fn build_worker_image() -> bool {
    ...
    let repo_root = match find_repo_root() {
        ...
        None => {
            eprintln!(
                "Warning: could not locate the repository root to build {HYDRA_WORKER_IMAGE}."
            );
            eprintln!(
    ...
    eprintln!();
    eprintln!("Building Docker image '{HYDRA_WORKER_IMAGE}' (this may take several minutes)...");
    ...
    match status {
        Ok(s) if s.success() => {
            eprintln!("Successfully built '{HYDRA_WORKER_IMAGE}'.");
            true
        }
        Ok(_) => {
            eprintln!();
...
fn prompt_github_pat() -> Result<String> {
    eprintln!();
    eprintln!("Hydra integrates with GitHub so agents can send you pull requests.");
...
fn render_server_config(
    ...
    let config = AppConfig {
        ...
        job: JobSection {
            default_image: HYDRA_WORKER_IMAGE.to_string(),
    ...
    format!("# Hydra server configuration (auto-generated by `hydra server init`)\n{yaml}")
}
...
fn wait_for_auth_token(token_path: &Path) -> Result<String> {
    ...
    bail!(
        "Timed out waiting for auth token at {}. \
         The server may have failed to start — check logs with `hydra server logs`.",
        token_path.display()
    );
}
...
fn verify_auth_token(auth_token: &str) -> Result<()> {
    ...
    rt.block_on(client.whoami()).context(
        "auth token verification failed — the server rejected the token. \
         Try removing ~/.hydra/server and running `hydra server init` again.",
    )?;
    Ok(())
}
...
fn wait_for_server_healthy() -> Result<()> {
    ...
    bail!(
        "Server failed to start — /health did not respond within 30 seconds. \
         Check logs with `hydra server logs`."
    );
}
...
fn start_server_in_process() -> Result<()> {
    ...
    // Set the server config env var so `hydra_server::run()` can find it.
    ...
    #[cfg(not(unix))]
    {
        bail!(
            "In-process server startup via fork is only supported on Unix. \
             Please run `hydra-server` as a separate process."
        );
    }
...
/// Start the hydra server with the BFF layer (auth routes, API proxy,
...
async fn run_server_with_bff() -> Result<()> {
    ...
    let state = hydra_server::build_app_state(app_config).await?;
    ...
    // Build the internal hydra-server router with state applied.
    let inner_app = hydra_server::build_router(&state).with_state(state.clone());

    let bff_app = crate::bff::build_bff_router(inner_app, auto_login_token);

    let listener = tokio::net::TcpListener::bind("0.0.0.0:8080").await?;

    hydra_server::run_with_state(state, listener, bff_app).await
}
...
fn cmd_status() -> Result<()> {
    ...
    if !server_dir.exists() {
        println!("Server is not initialized. Run `hydra server init` first.");
        return Ok(());
    }
...
fn ensure_initialized() -> Result<()> {
    let config_path = expand_path(SERVER_CONFIG_PATH);
        ...
        config_path.exists(),
        "Server is not initialized. Run `hydra server init` first."
    );
    Ok(())
}
...
mod tests {
    ...
    fn render_server_config_contains_required_fields() {
        ...
        assert!(config.contains("# Hydra server configuration"));
        ...
        assert_eq!(app_config.job.default_image, HYDRA_WORKER_IMAGE);
        assert!(app_config.job.default_model.is_none());
        assert!(app_config.hydra.openai_api_key.is_none());
        assert!(app_config.hydra.anthropic_api_key.is_none());
        assert!(app_config.hydra.claude_code_oauth_token.is_none());
        assert!(matches!(
            app_config.storage,
            hydra_server::config::StorageConfig::Sqlite { .. }
        ));
        assert!(matches!(
    ...
    fn render_server_config_local_engine() {
        ...
        match &app_config.job_engine {
            hydra_server::config::JobEngineConfig::Local { log_dir } => {
                assert_eq!(*log_dir, Some("/custom/log/dir".to_string()));
            }
            other => panic!("expected JobEngineConfig::Local, got {other:?}"),
        }
        assert_eq!(app_config.hydra.server_hostname, "127.0.0.1:8080");
    }
    ...
    fn render_server_config_with_codex_model_and_openai_key() {
        ...
        assert_eq!(
            app_config.hydra.openai_api_key.as_deref(),
    ...
    fn render_server_config_with_claude_model_and_anthropic_key() {
        ...
        assert_eq!(
            app_config.hydra.anthropic_api_key.as_deref(),
    ...
    fn render_server_config_with_claude_oauth_token() {
        ...
        assert_eq!(app_config.job.default_model.as_deref(), Some("opus"));
        assert_eq!(
            app_config.hydra.claude_code_oauth_token.as_deref(),
    ...
    fn ensure_initialized_fails_when_not_initialized() {
        ...
        // This will fail or succeed depending on whether ~/.hydra/server/config.yaml exists.
