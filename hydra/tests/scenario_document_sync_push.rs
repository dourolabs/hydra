...
///   2. A second worker runs `hydra documents sync` (no directory argument,
///      relying on the `HYDRA_DOCUMENTS_DIR` env var set by `worker_run`).
///   3. The worker verifies the synced file exists, edits it in-place.
///   4. The worker runs `hydra documents push` (no directory argument).
...
async fn document_sync_push_through_worker() -> Result<()> {
    ...
    let repo = hydra_common::RepoName::from_str("acme/doc-sync")?;
