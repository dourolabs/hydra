...
Summarizes an issue along with its immediate parents, transitive children, linked patches, and the complete activity log. Use `--verbose` to emit the full JSONL payload (including expanded parent/child records and raw activity entries) for automation or auditing.
