...
pub struct Cli {
    ...
    /// Override the Hydra server URL (also via HYDRA_SERVER_URL).
    #[arg(
        long = "server-url",
        value_name = "URL",
        env = ENV_HYDRA_SERVER_URL,
        global = true
    )]
...
pub fn load_app_config(config_path: &Path) -> Result<AppConfig> {
    ...
    if !resolved_path.exists() {
        anyhow::bail!(
            "No configuration file found at '{}'. Run 'hydra server init' or create a config file with your server URL.",
