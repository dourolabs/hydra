//! Constants for file paths and directory names used throughout the hydra CLI.
//!
...
/// Base directory for CLI assets (`~/.local/share/hydra`).
pub const HYDRA_DIR: &str = "~/.local/share/hydra";

/// The output text file name (`output.txt`) used when capturing codex output for a job.
pub const OUTPUT_TXT_FILE: &str = "output.txt";

/// The default configuration file path (`~/.local/share/hydra/config.toml`) used when no
/// config file is explicitly specified via the `--config` flag.
pub const DEFAULT_CONFIG_FILE: &str = "~/.local/share/hydra/config.toml";
