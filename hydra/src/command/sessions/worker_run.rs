...
pub async fn run(
    ...
    // surfaced on the worker subprocess's stdout/stderr, which the job engine
    // captures into the per-session log file. `try_init` is a no-op if a
    // subscriber has already been installed (e.g. inside an integration test
    // that initializes its own).
    ...
    let last_message = match selector_result {
        Err(err) => {
    ...
    // useful end. The kill-process-group path in `worker::claude` only
