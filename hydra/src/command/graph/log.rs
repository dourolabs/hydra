...
async fn events_for_one(
    ...
    Ok(events_in_window(&id, kind, &versions, since, until, verbosity))
}
...
mod tests {
    ...
    fn events_in_window_emits_created_then_updated_for_in_window_versions() {
        ...
        let events =
            events_in_window(&hydra_id, ObjectKind::Issue, &history, ts(0), ts(100), VerbosityLevel::L3);
        // v2, v3, v4 are inside the window (3 events).
        assert_eq!(events.len(), 3, "got {events:?}");
        // All should be Updated (since prev exists for each).
        for event in &events {
            assert!(matches!(event, LogEvent::Updated { .. }), "expected Updated: {event:?}");
        }
    }
    ...
    fn events_in_window_emits_created_when_earliest_in_window_is_first_version() {
        ...
        let hydra_id: HydraId = id.clone().into();
        let events =
            events_in_window(&hydra_id, ObjectKind::Issue, &history, ts(0), ts(100), VerbosityLevel::L1);
    ...
    fn events_in_window_skips_versions_outside_range() {
        ...
        let history = VersionedNode::Issue(vec![
            issue_version_record(&id, 1, ts(-100), sample_issue(IssueStatus::Open, "t1"), None),
            issue_version_record(&id, 2, ts(1000), sample_issue(IssueStatus::Closed, "t1"), None),
        ]);
        let hydra_id: HydraId = id.clone().into();
    ...
    fn events_in_window_excludes_boundary_since_includes_until() {
        ...
        let history = VersionedNode::Issue(vec![
            ...
            issue_version_record(&id, 2, ts(100), sample_issue(IssueStatus::InProgress, "t"), None),
        ]);
        let hydra_id: HydraId = id.clone().into();
        let events =
            events_in_window(&hydra_id, ObjectKind::Issue, &history, ts(0), ts(100), VerbosityLevel::L1);
