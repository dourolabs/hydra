...
use hydra_common::{
    ...
    users::Username,
    ActivityLogEntry, ActivityObjectKind, HydraId, LabelId, PatchId, RelativeVersionNumber,
...
async fn fetch_patch_versions(
    client: &dyn HydraClientInterface,
    patch_id: &PatchId,
) -> Result<Vec<Versioned<hydra_common::patches::Patch>>> {
    let response = client
...
mod tests {
    ...
    use hydra_common::issues::{
        Issue, IssueSummaryRecord, IssueVersionRecord, ListIssueVersionsResponse,
        ListIssuesResponse, SessionSettings, UpsertIssueRequest, UpsertIssueResponse,
    };
    use hydra_common::{
    ...
    async fn write_issue_details_pretty_shows_labels() {
        ...
        let mut output = Vec::new();
        write_issue_details_pretty(&issue_with_patches, "", &mut output).unwrap();
        let rendered = strip_ansi_codes(&String::from_utf8(output).unwrap());
