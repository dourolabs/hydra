...
pub struct SyncArgs {
    ...
    #[arg(value_name = "DIRECTORY", env = ENV_HYDRA_DOCUMENTS_DIR)]
...
#[derive(Debug, Clone, Args)]
pub struct PushArgs {
    /// Local directory previously synced with `hydra documents sync`.
    #[arg(value_name = "DIRECTORY", env = ENV_HYDRA_DOCUMENTS_DIR)]
    pub directory: Option<PathBuf>,
...
pub async fn run(
    ...
    match command {
        DocumentsCommand::List(args) => {
            ...
            write_document_summaries_output(context.output_format, &documents)?;
        }
...
fn write_document_summaries_output(
    format: ResolvedOutputFormat,
...
async fn create_document(
    ...
    let parsed_path = if let Some(path) = args.path.as_deref() {
        ...
        Some(
            path.parse::<hydra_common::DocumentPath>()
...
async fn update_document(
    ...
    if args.clear_path {
        ...
        let parsed: hydra_common::DocumentPath = path
            .parse()
            .context("document path must not contain hidden segments (starting with '.')")?;
...
const MANIFEST_FILENAME: &str = ".hydra-documents.json";
...
pub async fn sync_documents(client: &dyn HydraClientInterface, args: SyncArgs) -> Result<()> {
    ...
    for summary in &pathed_documents {
        let doc_path = summary.document.path.as_deref().unwrap();
        ...
        if let Some(ref manifest) = existing_manifest {
            if let Some(existing_entry) = manifest.documents.get(relative_path) {
                if existing_entry.version == summary.version
                    && existing_entry.document_id == summary.document_id
                {
        ...
        // Fetch the full document to get body_markdown
        let full_record = client
            .get_document(&summary.document_id, false)
            .await
            .with_context(|| {
                format!(
                    "failed to fetch document '{}' for sync",
                    summary.document_id
                )
            })?;
...
pub async fn push_documents(client: &dyn HydraClientInterface, args: PushArgs) -> Result<()> {
    ...
    let manifest = load_manifest(directory)?.with_context(|| {
        format!(
            "no manifest file found at '{}'. Run 'hydra documents sync' first.",
...
mod tests {
    ...
    fn sample_document_summary_record(id: &DocumentId) -> DocumentSummaryRecord {
        DocumentSummaryRecord::from(&sample_document_record(id))
    }
    ...
    async fn list_documents_supports_filters() {
        ...
        let response =
            ListDocumentsResponse::new(vec![sample_document_summary_record(&document_id)]);
    ...
    async fn sync_documents_clean_removes_stale_files() {
        ...
        .unwrap();
        let removed_record = DocumentVersionRecord::new(
            removed_id.clone(),
            0,
            Utc::now(),
            removed_doc,
        ...
        let response2 = ListDocumentsResponse::new(vec![DocumentSummaryRecord::from(&record)]);
        mock1.delete();
    ...
    async fn sync_documents_with_path_prefix_filter() {
        ...
        server.mock(|when, then| {
            when.method(GET)
                .path(format!("/v1/documents/{}", record.document_id));
            then.status(200).json_body_obj(&record);
        });
        let client = mock_client(&server);
