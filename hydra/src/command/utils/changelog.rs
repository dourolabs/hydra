...
pub struct ActivityLogEntrySummary {
    ...
    pub actor: Option<hydra_common::actor_ref::ActorRef>,
}
...
pub fn summarize_activity_object(entry: &ActivityLogEntry) -> Result<ActivityObjectSummary> {
    match entry.object_kind {
        ...
        ActivityObjectKind::Patch => {
            let patch: hydra_common::patches::Patch = decode_activity_object(entry)?;
        ...
        ActivityObjectKind::Document => {
            let doc: hydra_common::api::v1::documents::Document = decode_activity_object(entry)?;
...
pub fn summarize_activity_changes(
    ...
    let mut before_patch: Option<hydra_common::patches::Patch> = None;
    let mut after_patch: Option<hydra_common::patches::Patch> = None;
...
fn format_actor_label(actor: &hydra_common::actor_ref::ActorRef) -> Option<String> {
