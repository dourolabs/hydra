...
mod tests {
    ...
    const DEFAULT_FILE_PATH: &str = "file.txt";
    const DEFAULT_INITIAL_SPEC: &str = "file.txt:initial\n";
    ...
    impl ScriptedRepo {
        fn new() -> Result<Self> {
            ...
            let initial = initial_commit(&repo, DEFAULT_INITIAL_SPEC)?;
    ...
    fn try_append_advances_tip_and_records_patches() -> Result<()> {
        let scripted = ScriptedRepo::new()?;
        let (base_commit, _) = scripted.base_from([
            format!("{DEFAULT_FILE_PATH}:base\n"),
            format!("{DEFAULT_FILE_PATH}:base updated\n"),
        ])?;

        let patches = scripted.commit_chain(
            base_commit,
            [
                format!("{DEFAULT_FILE_PATH}:from patch1\n"),
                format!("{DEFAULT_FILE_PATH}:from patch2\n"),
            ],
        )?;
        let patch1 = patches[0];
        let patch2 = patches[1];

        let queue_repo = scripted.queue_repo()?;
        ...
        assert_eq!(
            file_at_commit(scripted.repo(), queue.tip(), DEFAULT_FILE_PATH)?,
            "from patch2\n"
        );
    ...
    fn try_append_rejects_unmergeable_patch() -> Result<()> {
        ...
        let patch1 = scripted
            .commit_chain(base_commit, [format!("{DEFAULT_FILE_PATH}:left change\n")])?
            .pop()
            .unwrap();
        let patch2 = scripted
            .commit_chain(base_commit, [format!("{DEFAULT_FILE_PATH}:right change\n")])?
            .pop()
            .unwrap();
    ...
    fn try_append_handles_divergent_branches_with_additional_files() -> Result<()> {
        ...
        let (base, base_history) = scripted.base_from([
            format!("{DEFAULT_FILE_PATH}:base v1\n"),
            format!("{DEFAULT_FILE_PATH}:base v2\n"),
            format!("{DEFAULT_FILE_PATH}:base v3\n"),
        ])?;
        ...
        let hotfix =
            scripted.commit_chain(base, [format!("{DEFAULT_FILE_PATH}:base v3 hotfix\n")])?;
    ...
    fn evict_removes_patch_and_reapplies_queue() -> Result<()> {
        let scripted = ScriptedRepo::new()?;
        let (base, _) = scripted.base_from([format!("{DEFAULT_FILE_PATH}:base branch\n")])?;

        let patch1 = scripted
            .commit_chain(base, [format!("{DEFAULT_FILE_PATH}:aligned value\n")])?
            .pop()
            .unwrap();
        let patch2 = scripted
            .commit_chain(
                scripted.initial,
                [format!("{DEFAULT_FILE_PATH}:aligned value\n")],
            )?
            .pop()
            .unwrap();
    ...
    fn evict_replays_dependent_branch_and_preserves_predecessors() -> Result<()> {
        ...
        let feature_patch = scripted
            ...
            .unwrap();
        let hotfix = scripted
            .commit_chain(base, [format!("{DEFAULT_FILE_PATH}:base v2 hotfix\n")])?
            .pop()
            .unwrap();
        let hotfix_follow_up = scripted
            .commit_chain(
                hotfix,
                [format!("{DEFAULT_FILE_PATH}:base v2 hotfix follow up\n")],
            )?
            .pop()
            .unwrap();
