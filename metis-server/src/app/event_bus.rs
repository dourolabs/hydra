...
/// Actor name used for background/system-initiated operations where no
/// human user is responsible for the mutation.
...
mod tests {
    ...
    async fn store_with_events_emits_on_add_issue() {
        ...
        let (issue_id, _) = store.add_issue_with_actor(issue, "test".to_string()).await.unwrap();
    ...
    async fn store_with_events_emits_on_update_issue() {
        ...
        let (issue_id, _) = store
            .add_issue_with_actor(issue.clone(), "test".to_string())
            .await
            .unwrap();
        ...
        store
            .update_issue_with_actor(&issue_id, updated, "test".to_string())
            .await
            .unwrap();
    ...
    async fn job_update_event_carries_old_and_new_payload() {
        ...
        let (task_id, _) = store
            .add_task_with_actor(task, chrono::Utc::now(), "test".to_string())
            .await
            .unwrap();
        ...
        store
            .update_task_with_actor(&task_id, updated_task, "test".to_string())
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn actor_context_is_system_when_not_explicitly_set() {
        use crate::domain::issues::{Issue, IssueStatus, IssueType};
        use crate::domain::users::Username;
        ...
        match &event {
            ServerEvent::IssueCreated { payload, .. } => {
                assert_eq!(
                    payload.actor(),
                    SYSTEM_ACTOR,
                    "actor should be 'system' when no user context is set"
                );
            }
            other => panic!("expected IssueCreated, got {other:?}"),
        }
