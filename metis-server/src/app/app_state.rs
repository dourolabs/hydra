    ...
    pub async fn upsert_issue(
        ...
        let issue_id = match issue_id {
            ...
            None => {
                ...
                if issue.creator.username.as_str().trim().is_empty() {
...
mod tests {
    ...
    async fn create_issue_inherits_creator_from_parent_when_empty() {
        ...
        let parent_creator = User {
            username: "parent-creator".into(),
            github_user_id: None,
            github_token: "token".to_string(),
        };
        parent_issue.creator = parent_creator.clone();
        let parent_id = state
            .upsert_issue(None, UpsertIssueRequest::new(parent_issue, None))
            .await
            .unwrap();
        ...
        assert_eq!(stored_child.creator, parent_creator);
    }
    ...
    async fn create_issue_preserves_explicit_creator_with_parent() {
        ...
        let explicit_creator = User {
            username: "explicit-creator".into(),
            github_user_id: None,
            github_token: "token".to_string(),
        };
        child_issue.creator = explicit_creator.clone();
        let child_id = state
            .upsert_issue(None, UpsertIssueRequest::new(child_issue, None))
            .await
            .unwrap();
        ...
        let store = state.store.read().await;
        let stored_child = store.get_issue(&child_id).await.unwrap();
        assert_eq!(stored_child.creator, explicit_creator);
    }
    ...
    async fn create_issue_without_parent_keeps_empty_creator() {
        ...
        assert!(stored_issue.creator.username.as_str().is_empty());
    }
}
