...
impl AppState {
    ...
    pub async fn upsert_patch(
        ...
        // Transfer sync_github_branch from the request to the patch so
        // that the github_pr_sync automation can pick it up after
        // persistence.
        if sync_github_branch.is_some() {
            if actor.is_none() {
                return Err(UpsertPatchError::GithubActorMissing);
            }
            patch.sync_github_branch = sync_github_branch;
        }
        ...
        let (patch_id, version) = match patch_id {
            Some(id) => {
                ...
                if patch.sync_github_branch.is_some() && patch.github.is_none() {
...
mod tests {
    ...
    async fn upsert_patch_sync_github_updates_existing_pr() -> anyhow::Result<()> {
        ...
        // sync_github_branch should be cleared after automation runs.
        assert!(stored_patch.item.sync_github_branch.is_none());
