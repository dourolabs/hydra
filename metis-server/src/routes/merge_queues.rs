...
use metis_common::merge_queues::{EnqueueMergePatchRequest, MergeQueue};
use tracing::{error, info};

pub async fn get_merge_queue(
    State(state): State<AppState>,
    Path((service_repo_name, branch_name)): Path<(String, String)>,
) -> Result<Json<MergeQueue>, ApiError> {
    info!(
        service_repo = %service_repo_name,
        branch = %branch_name,
        "get_merge_queue invoked"
    );
    let queue = state
        .merge_queue(&service_repo_name, &branch_name)
        .await
...
pub async fn enqueue_patch(
    ...
    let queue = state
        .enqueue_merge_queue_patch(&service_repo_name, &branch_name, request.patch_id)
        .await
        .map_err(map_merge_queue_error)?;
