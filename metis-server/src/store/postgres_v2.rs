...
mod tests {
    ...
    #[sqlx::test(migrations = "./migrations")]
    #[ignore]
    async fn task_search_only_matches_latest_version_v2(pool: PgStorePool) {
        let store = PostgresStoreV2::new(pool);

        // Create a task with a unique prompt
        let task = Task::new(
            "original_unique_prompt_abc123".to_string(),
            BundleSpec::None,
            None,
            Some("metis-worker:latest".to_string()),
            None,
        ...
        // Update the task to change the prompt
        let updated_task = Task::new(
            "changed_unique_prompt_xyz789".to_string(),
            BundleSpec::None,
            None,
            Some("metis-worker:latest".to_string()),
            None,
        ...
        // Search for the old prompt - should return NO results
        let old_query = SearchJobsQuery::new(
            Some("original_unique_prompt_abc123".to_string()),
            None,
            Some(false),
        );
        let old_results = store.list_tasks(&old_query).await.unwrap();
        assert!(
            old_results.is_empty(),
            "Search for old prompt should return no results, but got {:?}",
            old_results.iter().map(|(id, _)| id).collect::<Vec<_>>()
        );

        // Search for the new prompt - should return the task
        let new_query = SearchJobsQuery::new(
            Some("changed_unique_prompt_xyz789".to_string()),
            None,
            Some(false),
        );
        let new_results = store.list_tasks(&new_query).await.unwrap();
        assert_eq!(new_results.len(), 1);
        assert_eq!(new_results[0].0, task_id);
        assert_eq!(new_results[0].1.item.prompt, "changed_unique_prompt_xyz789");
    }
}
