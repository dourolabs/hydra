...
fn paginate_items<Id: AsRef<str> + Clone, T: Clone>(
    items: &mut Vec<(Id, Versioned<T>)>,
...
impl ReadOnlyStore for PostgresStoreV2 {
    ...
    async fn list_patches_paginated(
        ...
        let (next_cursor, total_count) = paginate_items(&mut items, pagination);
    ...
    async fn list_issues_paginated(
        ...
        let all_issue_ids: std::collections::HashSet<IssueId> =
            all_issues.iter().map(|(id, _)| id.clone()).collect();
        ...
        let selected_root_ids: std::collections::HashSet<IssueId> =
            roots.iter().map(|(id, _)| id.clone()).collect();

        let mut children_map: std::collections::HashMap<IssueId, Vec<IssueId>> =
        ...
        let mut result_ids: std::collections::HashSet<IssueId> = selected_root_ids.clone();
