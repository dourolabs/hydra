    ...
    async fn list_tasks(
        ...
        Ok(self
            ...
            .filter_map(|entry| {
                ...
                // Filter by search term (id, prompt, status)
                if let Some(ref term) = search_term {
                    if !task_matches_search(task_id, &latest.item, term) {
                        return None;
                    }
                }
...
/// Checks if the task matches the search term by looking at id, prompt, and status.
fn task_matches_search(task_id: &TaskId, task: &Task, term: &str) -> bool {
    // Match against task id
    if task_id.to_string().to_lowercase().contains(term) {
        return true;
    }

    // Match against prompt
    if task.prompt.to_lowercase().contains(term) {
        return true;
    }

    // Match against status (format status as debug string like "Complete", "Running", etc.)
    let status_str = format!("{:?}", task.status).to_lowercase();
    if status_str.contains(term) {
        return true;
    }
...
mod tests {
    ...
    async fn list_tasks_filters_by_search_term() {
        ...
        // Search for "registration" should find task2
        let query = SearchJobsQuery::new(Some("registration".to_string()), None, None);
        let tasks: Vec<_> = store
            .list_tasks(&query)
            .await
            .unwrap()
        ...
        let tasks: HashSet<_> = store
            ...
            .map(|(id, _)| id)
            .collect();
        assert_eq!(tasks, HashSet::from([task1_id.clone(), task2_id.clone()]));
    }
    ...
    async fn list_tasks_search_term_matches_task_id() {
        ...
        // Search by task ID prefix should find the task
        let id_prefix = task_id.to_string()[..4].to_string();
        let query = SearchJobsQuery::new(Some(id_prefix), None, None);
        let tasks: Vec<_> = store
            .list_tasks(&query)
            .await
            .unwrap()
    ...
    async fn list_tasks_search_term_matches_status() {
        ...
        // Create a task with Created status (default)
        let task1 = spawn_task();
        let task1_id = store.add_task(task1, Utc::now()).await.unwrap();

        // Create a task and update it to Complete status
        let task2 = spawn_task();
        let task2_id = store.add_task(task2, Utc::now()).await.unwrap();
        let mut task2_updated = store.get_task(&task2_id).await.unwrap().item;
        task2_updated.status = Status::Complete;
        store.update_task(&task2_id, task2_updated).await.unwrap();

        // Search for "created" should find task1
        ...
        // Search for "complete" should find task2
