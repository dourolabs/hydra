...
async fn list_users_does_not_return_tokens() -> anyhow::Result<()> {
    ...
    assert_eq!(
        user.get("github_username").and_then(Value::as_str),
        Some("alice-gh")
    );
    assert_eq!(
...
async fn set_github_token_overwrites_existing() -> anyhow::Result<()> {
    ...
    let payload = CreateUserRequest {
        ...
        github_username: Some("bob-gh".to_string()),
    ...
    let update_payload = UpdateGithubTokenRequest {
        ...
        github_username: Some("bob-updated".to_string()),
    };
    ...
    assert_eq!(users[0].github_username.as_deref(), Some("bob-updated"));
