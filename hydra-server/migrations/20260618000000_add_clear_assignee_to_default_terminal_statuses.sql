...
--
-- This is PR-1 of the chain that replaces the historical
-- `unblocks_parents` "terminal issue" proxy with explicit
-- `StatusOnEnter` flags. The cutover that drops the proxy itself lands
-- in a follow-up PR; this migration ships the new flag ahead of that
-- cutover so the assign-clearing behavior is in place when the proxy
-- goes away.
--
