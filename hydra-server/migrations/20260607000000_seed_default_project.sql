...
-- pre-existing `metis.issues_v2.project_id IS NULL` rows so the
-- consumer side can drop its in-process `default_project()` fallback
-- (see issue [[i-dqzrijzy]]).
--
...
--
-- The migration is idempotent (ON CONFLICT DO NOTHING). The seeded
...
-- Backfill: every legacy metis.issues_v2 row (any version) with NULL
-- project_id now points at the seeded default project.
UPDATE metis.issues_v2 SET project_id = 'j-defaul' WHERE project_id IS NULL;
