...
ALTER TABLE hydra.tasks_v2 ADD COLUMN IF NOT EXISTS creation_time TIMESTAMPTZ;
ALTER TABLE hydra.tasks_v2 ADD COLUMN IF NOT EXISTS start_time TIMESTAMPTZ;
ALTER TABLE hydra.tasks_v2 ADD COLUMN IF NOT EXISTS end_time TIMESTAMPTZ;

-- Backfill timing on the latest version of each task.
-- creation_time: timestamp of the first version (version_number = 1).
UPDATE hydra.tasks_v2 t
...
WHERE t.id = sub.id
  ...
  AND t.version_number = (
      SELECT MAX(version_number) FROM hydra.tasks_v2 WHERE id = t.id
  );
