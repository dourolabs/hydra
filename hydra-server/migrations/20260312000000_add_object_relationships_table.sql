...
CREATE TABLE IF NOT EXISTS hydra.object_relationships (
...
CREATE INDEX IF NOT EXISTS object_relationships_target_idx
    ON hydra.object_relationships (target_id, rel_type);
CREATE INDEX IF NOT EXISTS object_relationships_source_idx
    ON hydra.object_relationships (source_id, rel_type);

-- Backfill issue dependencies (child-of, blocked-on) from JSONB
INSERT INTO hydra.object_relationships (source_id, source_kind, target_id, target_kind, rel_type)
...
FROM hydra.issues_v2 i,
     LATERAL jsonb_array_elements(i.dependencies) AS dep
WHERE i.version_number = (
    SELECT MAX(version_number) FROM hydra.issues_v2 WHERE id = i.id
)
AND NOT i.deleted
