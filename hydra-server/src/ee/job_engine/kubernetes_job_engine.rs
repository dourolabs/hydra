...
fn merge_env_vars(
    ...
    merged_vars.insert(ENV_HYDRA_ID.to_string(), job_uuid.to_string());
    merged_vars.insert(ENV_HYDRA_TOKEN.to_string(), auth_token.to_string());

    let hostname = server_hostname.trim();
    if !hostname.is_empty() {
        merged_vars.insert(
            ENV_HYDRA_SERVER_URL.to_string(),
...
impl KubernetesJobEngine {
    ...
    fn build_env_vars(
        ...
        merge_env_vars(job_uuid, env_vars, &self.server_hostname, auth_token)
    }
    ...
    fn job_hydra_id(job: &Job) -> Option<SessionId> {
        job.metadata
            .labels
            .as_ref()
    ...
    fn to_hydra_job(job: &Job) -> Result<HydraJob, JobEngineError> {
        let job_name = job
            .metadata
            .name
            .clone()
            .unwrap_or_else(|| "<unknown>".to_string());
        let id = Self::job_hydra_id(job).ok_or_else(|| {
            JobEngineError::Internal(format!("Job '{job_name}' is missing hydra-id label"))
        })?;
    ...
    fn pod_hydra_id(pod: &Pod) -> Option<SessionId> {
        pod.metadata
            .labels
            .as_ref()
            .and_then(|labels| labels.get("metis-id"))
    ...
    fn to_hydra_job_from_pod(pod: &Pod) -> Result<HydraJob, JobEngineError> {
        let pod_name = pod
            .metadata
            .name
            .clone()
            .unwrap_or_else(|| "<unknown>".to_string());
        let id = Self::pod_hydra_id(pod).ok_or_else(|| {
            JobEngineError::Internal(format!("Pod '{pod_name}' is missing hydra-id label"))
        })?;
    ...
    async fn find_kubernetes_job_by_hydra_id(
        &self,
        job_id: &SessionId,
    ) -> Result<Job, JobEngineError> {
        find_kubernetes_job_by_hydra_id_impl(&self.client, &self.namespace, job_id).await
    }

    async fn find_kubernetes_pod_by_hydra_id(
        &self,
        job_id: &SessionId,
    ) -> Result<Pod, JobEngineError> {
        find_kubernetes_pod_by_hydra_id_impl(&self.client, &self.namespace, job_id).await
    }
    ...
    async fn kill_pods_by_hydra_id(&self, hydra_id: &SessionId) -> Result<(), JobEngineError> {
        ...
        let pod_list = pods
            ...
            .await
            .map_err(|err| {
                error!(hydra_id = %hydra_id, error = ?err, namespace = %self.namespace, "failed to list pods before deletion");
        ...
        if deleted_any {
            ...
            Err(JobEngineError::NotFound(hydra_id.clone()))
        }
    }
}

async fn find_kubernetes_job_by_hydra_id_impl(
    client: &Client,
    namespace: &str,
    job_id: &SessionId,
) -> Result<Job, JobEngineError> {
...
async fn find_kubernetes_pod_by_hydra_id_impl(
    client: &Client,
    namespace: &str,
    job_id: &SessionId,
) -> Result<Pod, JobEngineError> {
...
async fn resolve_pod_name_impl(
    ...
    match find_kubernetes_job_by_hydra_id_impl(client, namespace, job_id).await {
        Ok(job) => {
        ...
        Err(JobEngineError::NotFound(_)) => {
            let mut pod = find_kubernetes_pod_by_hydra_id_impl(client, namespace, job_id).await?;
...
impl JobEngine for KubernetesJobEngine {
    async fn create_job(
        ...
        let metadata_labels = Self::build_metadata_labels(hydra_id);
        ...
        let mut container = Container {
            ...
            env: Some(self.build_env_vars(hydra_id, env_vars, auth_token)),
            ..Default::default()
        };
        ...
        match jobs.create(&pp, &job).await {
            ...
            Err(kube::Error::Api(err)) if err.code == 409 => {
                ...
                Err(JobEngineError::AlreadyExists(hydra_id.clone()))
            }
            Err(err) => {
    ...
    async fn list_jobs(&self) -> Result<Vec<HydraJob>, JobEngineError> {
        ...
        let mut hydra_jobs: Vec<HydraJob> = jobs
            .into_iter()
            .filter_map(|job| Self::to_hydra_job(&job).ok())
            .collect();
        let mut seen_ids: HashSet<SessionId> =
            hydra_jobs.iter().map(|job| job.id.clone()).collect();
            ...
            Ok(job) => {
                let job_name = job.metadata.name.ok_or_else(|| {
                    JobEngineError::Internal(format!(
                        "Job '{hydra_id}' is missing a Kubernetes name."
                    ))
                })?;
                ...
                match jobs.delete(&job_name, &dp).await {
                    Ok(_) => {
                        ...
                        if let Err(err) = self.kill_pods_by_hydra_id(hydra_id).await {
                    ...
                    Err(kube::Error::Api(err)) if err.code == 404 => {
                        self.kill_pods_by_hydra_id(hydra_id).await
                    }
                    Err(err) => {
                        error!(
                            hydra_id = %hydra_id,
...
mod tests {
    ...
    fn merge_env_vars_combines_task_and_system_values() {
        ...
        task_env.insert(ENV_HYDRA_ID.to_string(), "override-me".to_string());
        task_env.insert(
            ENV_HYDRA_SERVER_URL.to_string(),
        ...
        assert_eq!(
            merged_map.get("OPENAI_API_KEY"),
            Some(&"openai-key".to_string())
        );
        assert_eq!(
            merged_map.get("ANTHROPIC_API_KEY"),
            Some(&"anthropic-key".to_string())
        );
        assert_eq!(
            merged_map.get("CLAUDE_CODE_OAUTH_TOKEN"),
            Some(&"claude-token".to_string())
        );
        assert_eq!(merged_map.get(ENV_HYDRA_ID), Some(&job_id.to_string()));
        assert_eq!(
            merged_map.get(ENV_HYDRA_SERVER_URL),
            Some(&"http://hydra.example.com".to_string())
        );
        assert_eq!(
            merged_map.get(ENV_HYDRA_TOKEN),
    ...
    fn merge_env_vars_skips_empty_server_hostname() {
        ...
        assert!(!merged_map.contains_key(ENV_HYDRA_SERVER_URL));
    }
    ...
    #[test]
    fn to_hydra_job_from_pod_uses_pod_metadata() {
        let job_id: SessionId = "s-abcd".parse().unwrap();
        ...
        let hydra_job =
            KubernetesJobEngine::to_hydra_job_from_pod(&pod).expect("conversion should succeed");

        assert_eq!(hydra_job.id, job_id);
        assert_eq!(hydra_job.status, JobStatus::Complete);
        assert_eq!(hydra_job.creation_time, Some(creation.0));
        assert_eq!(hydra_job.start_time, Some(start.0));
        assert_eq!(hydra_job.completion_time, Some(finished.0));
    }
    ...
    fn pod_failure_message_prefers_terminated_message() {
        ...
        assert_eq!(hydra_job.failure_message.as_deref(), Some("boom"));
        assert_eq!(hydra_job.status, JobStatus::Failed);
    }
}
