...
/// Automatically applies the 'inbox' label to issues when a human user creates
/// an issue (IssueCreated with Authenticated Username actor).
...
impl Automation for InboxLabelAutomation {
    ...
    async fn execute(&self, ctx: &AutomationContext<'_>) -> Result<(), AutomationError> {
        match ctx.event {
            ServerEvent::IssueCreated { payload, .. } if is_human_actor(payload.actor()) => {
...
mod tests {
    ...
    async fn setup_inbox_label(handles: &test_utils::TestStateHandles) -> hydra_common::LabelId {
        handles
            .state
            .create_label(
    ...
    #[tokio::test]
    async fn does_not_apply_on_create_with_human_assignee_but_system_actor() {
        let handles = test_utils::test_state_handles();
        let store = handles.store.clone();
