...
use hydra_common::{IssueId, SearchRepositoriesQuery, SessionId};
...
impl CleanupBranchesWorker {
    async fn cleanup_repo_branches(
        ...
        let refs = list_hydra_refs(client, owner, repo).await?;
        ...
        let branches: Vec<HydraBranch> = refs
            .iter()
...
/// A parsed hydra tracking branch reference.
...
mod tests {
    ...
    fn parse_hydra_branch_task_head() {
        ...
        let branch = branch.unwrap();
        assert_eq!(branch.suffix, "head");
        assert!(matches!(branch.id_kind, HydraIdKind::Session(_)));
    }
    ...
    #[test]
    fn parse_hydra_branch_non_hydra_ref() {
        assert!(parse_hydra_branch("refs/heads/main").is_none());
        assert!(parse_hydra_branch("refs/heads/feature/foo").is_none());
    }
