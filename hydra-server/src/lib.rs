...
pub async fn build_app_state(app_config: AppConfig) -> anyhow::Result<AppState> {
    ...
    let secret_manager = Arc::new(
        SecretManager::from_base64(&app_config.hydra.secret_encryption_key)
            .context("invalid hydra.HYDRA_SECRET_ENCRYPTION_KEY")?,
    );
    ...
    let job_engine: Arc<dyn crate::job_engine::JobEngine> = match &app_config.job_engine {
        ...
        JobEngineConfig::Kubernetes { kubernetes } => {
            ...
            let engine = KubernetesJobEngine {
                namespace: app_config.hydra.namespace.clone(),
...
/// Build the base Axum router with all hydra API routes.
///
...
pub fn build_router(state: &AppState) -> Router<AppState> {
    ...
    let protected_routes = Router::new()
        ...
        .route(
            ...
            get(routes::sessions::relay::session_events_or_ws),
        )
...
pub async fn setup_local_auth(
    ...
    // story tidy (stale tokens from a previous run can never
