...
use hydra_common::api::v1::labels::{LabelSummary, SearchLabelsQuery};
use hydra_common::issues::IssueId;
use hydra_common::{LabelId, HydraId, Rgb};
...
impl AppState {
    ...
    pub async fn get_labels_for_objects(
        &self,
        object_ids: &[HydraId],
    ) -> Result<std::collections::HashMap<HydraId, Vec<LabelSummary>>, StoreError> {
    ...
    async fn cascade_label_recursive(
        ...
        for child_id in children {
            let object_id = HydraId::from(child_id.clone());
            self.store
                .add_label_association(label_id, &object_id)
                .await?;
...
mod tests {
    ...
    async fn add_and_get_label_association() {
        ...
        let label_id = state
            ...
            .await
            .unwrap();
        let object_id: HydraId = "i-testissue".parse().unwrap();
    ...
    async fn get_labels_for_objects_batch() {
        ...
        let obj1: HydraId = "i-issueone".parse().unwrap();
        let obj2: HydraId = "i-issuetwo".parse().unwrap();
        let obj3: HydraId = "i-issuethree".parse().unwrap();
    ...
    async fn upsert_issue_with_label_names_creates_and_assigns() {
        ...
        let object_id = HydraId::from(issue_id);
        let labels = state.get_labels_for_object(&object_id).await.unwrap();
        assert_eq!(labels.len(), 1);
    ...
    async fn cascade_label_to_single_level_children() {
        ...
        let label_id = state
            ...
            .await
            .unwrap();
        let parent_obj = HydraId::from(parent_id.clone());
        state
        ...
        let child_obj = HydraId::from(child_id);
    ...
    async fn cascade_label_to_multi_level_children() {
        ...
        let label_id = state
            ...
            .await
            .unwrap();
        let gp_obj = HydraId::from(grandparent_id.clone());
        state
        ...
        let parent_obj = HydraId::from(parent_id);
    ...
    async fn cascade_label_with_no_children_is_noop() {
        ...
        let label_id = state
            ...
            .await
            .unwrap();
        let obj = HydraId::from(issue_id.clone());
