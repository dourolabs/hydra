...
use hydra_common::{
    DocumentId, LabelId, MessageId, HydraId, NotificationId, PatchId, RepoName, SessionId,
...
impl StoreWithEvents {
    ...
    pub async fn update_session_with_actor(
        ...
        let old_session = self
            .inner
            .get_session(hydra_id, false)
            .await
            .ok()
            .map(|v| v.item);
        let new_session = session.clone();
        let result = self.inner.update_session(hydra_id, session, &actor).await?;
        let payload = Arc::new(MutationPayload::Session {
            old: old_session,
            new: new_session,
            actor,
        ...
        self.event_bus
            .emit_session_updated(hydra_id.clone(), result.version, payload);
        Ok(result)
    }
    ...
    async fn emit_issue_updated_for_relationship(
        ...
        for hydra_id in [source_id, target_id] {
            if let Some(issue_id) = hydra_id.as_issue_id() {
...
impl ReadOnlyStore for StoreWithEvents {
    ...
    async fn is_secret_internal(
        ...
        self.inner.is_secret_internal(username, secret_name).await
    }
}
...
mod tests {
    ...
    async fn add_label_association_emits_document_updated() {
        ...
        let object_id = HydraId::from(doc_id.clone());
        store
            .add_label_association(&label_id, &object_id)
            .await
            .unwrap();
    ...
    async fn label_association_on_unknown_entity_does_not_emit_event() {
        ...
        let object_id: HydraId = "p-fakepatch".parse().unwrap();
        store
            .add_label_association(&label_id, &object_id)
            .await
            .unwrap();
    ...
    async fn add_relationship_with_actor_emits_issue_updated_for_both_sides() {
        ...
        let source_id = HydraId::from(issue_id1.clone());
        let target_id = HydraId::from(issue_id2.clone());
