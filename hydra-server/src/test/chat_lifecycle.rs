...
/// Register an agent with a non-empty prompt document in the store. Used by
/// the chat-agent-prompt regression tests below to verify the end-to-end
/// prompt-prepend path on the worker's first Claude turn.
async fn register_agent_with_prompt_body(
    store: &Arc<dyn Store>,
    name: &str,
    prompt_body: &str,
) -> anyhow::Result<()> {
    use crate::domain::{actors::ActorRef, agents::Agent, documents::Document};
...
#[tokio::test]
async fn create_conversation_with_message_includes_first_message_in_worker_catch_up()
-> anyhow::Result<()> {
    // CLI flow (mirrors `hydra chat`): `POST /v1/conversations` with a
    // non-empty `message` and an agent that resolves to a non-empty prompt.
    // The worker that subsequently connects via the relay MUST observe the
    // first user message in its `WorkerCatchUp.events`, because
    // `feed_catch_up` is the only path on which `PromptPrepend` writes
    // `"{agent_prompt}\n\n{first_msg}"` to Claude's stdin on a fresh
    // conversation. `create_conversation` does NOT push the first message
    // via `chat_relay::send_to_worker`, so if the catch-up is empty the
    // worker has nothing to attach the agent prompt to and idle-times-out.
    //
    // This regression test asserts both prerequisites of the prepend rule:
    //   1. the session is created with the agent's non-empty prompt
    //   2. the worker's `Fresh` catch-up carries the first `UserMessage`
    ...
    assert_eq!(
        ...
        "session must carry the resolved agent prompt so the worker prepends it"
    );
    ...
    let user_msgs: Vec<&str> = catch_up
        .events
        .iter()
        .filter_map(|e| match e {
            ConversationEvent::UserMessage { content, .. } => Some(content.as_str()),
    ...
    assert_eq!(
        user_msgs,
        vec!["first user message"],
        "Fresh catch-up MUST contain the first user message; \
         on the racy main this can be empty when the spawn automation \
         beats the post-add append in create_conversation. Got {} events: {:?}",
        catch_up.events.len(),
        catch_up.events,
    );
...
#[tokio::test]
async fn create_conversation_emits_conversation_created_after_first_message_is_persisted()
-> anyhow::Result<()> {
    // The race: `SpawnConversationSessionsAutomation` triggers on
    // `ConversationCreated`. The first user message MUST be in the event
    // log AT THE TIME the bus event fires, otherwise the spawn → session →
    // worker chain produces a `WorkerCatchUp` with `events=[]` and the
    // first message never reaches Claude. We subscribe to the bus, snapshot
    ...
    let observer = tokio::spawn(async move {
        loop {
            match rx.recv().await {
                Ok(ServerEvent::ConversationCreated {
                    conversation_id, ..
                }) => {
                    return observer_store
                        .get_conversation_events(&conversation_id)
                        .await
                        .expect("get_conversation_events");
                }
                Ok(_) => continue,
                Err(_) => panic!("event bus closed before ConversationCreated"),
            }
        }
    });
    ...
    let user_messages: Vec<&str> = snapshot
        ...
        .filter_map(|v| match &v.item {
            DomainConversationEvent::UserMessage { content, .. } => Some(content.as_str()),
            _ => None,
        })
        .collect();
    assert!(
        user_messages.contains(&"first user message"),
        "ConversationCreated subscribers must see the first user message in \
         the conversation event log at the time the bus event fires. Got {} \
...
#[tokio::test]
async fn web_frontend_first_message_via_send_message_reaches_worker_via_relay() -> anyhow::Result<()>
{
    // Web flow (mirrors the frontend's `createConversation({})` +
    // `sendMessage` path): a conversation is created with no first message,
    // the worker connects and waits, and the first message arrives via
    // `POST /v1/conversations/:id/messages`, which DOES push through
    // `chat_relay::send_to_worker`. The worker observes the message via the
    // relay rather than the catch-up. This is the non-racy companion path
    // that we keep intact alongside the CLI fix.
    let (state, store) = state_with_idle_timeout_secs(60);
    let server = spawn_test_server_with_state(state.clone(), store.clone()).await?;
    let client = test_client();
    ...
    .await?;
    assert!(
        catch_up.events.is_empty(),
        "no first message yet, catch-up must be empty, got {:?}",
        catch_up.events,
    );
    ...
    let received = drain_server_messages(&mut ws, Duration::from_secs(2)).await?;
    let saw_user_msg = received.iter().any(|m| {
        matches!(
            m,
            ServerMessage::Event {
                event: ConversationEvent::UserMessage { content, .. }
            } if content == "first user message"
        )
    });
    assert!(
        saw_user_msg,
        "send_message after an empty createConversation must reach the worker \
         via chat_relay; got {received:?}"
    );
