...
#[tokio::test]
async fn bad_request_for_hydra_prefix() -> anyhow::Result<()> {
    let handles = test_state_with_secrets();
    let server = spawn_test_server_with_state(handles.state, handles.store).await?;
    let client = test_user_client();
...
fn test_state_with_secrets_and_config() -> TestStateHandles {
    ...
    config.hydra.openai_api_key = Some("global-openai-key".to_string());
    config.hydra.anthropic_api_key = Some("global-anthropic-key".to_string());
