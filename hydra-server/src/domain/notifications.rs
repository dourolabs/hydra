...
use hydra_common::{HydraId, IssueId, VersionNumber};
    ...
    async fn collect_recipients_for_issue(
        ...
        while let Some(current_id) = queue.pop_front() {
            ...
            if let Some(ref assignee) = issue.assignee {
                let username = hydra_common::api::v1::users::Username::from(assignee.clone());
...
pub fn event_object_kind(event: &ServerEvent) -> &'static str {
    match event {
        ...
        ServerEvent::ConversationCreated { .. } | ServerEvent::ConversationUpdated { .. } => {
            "conversation"
        }
    }
}

/// Extract the object ID as a HydraId from a server event.
pub fn event_object_id(event: &ServerEvent) -> HydraId {
    match event {
        ServerEvent::IssueCreated { issue_id, .. }
        | ServerEvent::IssueUpdated { issue_id, .. }
...
pub fn event_version(event: &ServerEvent) -> VersionNumber {
    match event {
        ...
        | ServerEvent::ConversationUpdated { version, .. } => *version,
    }
}
...
pub fn event_source_issue_id(event: &ServerEvent) -> Option<IssueId> {
    ...
    match payload.as_ref() {
        ...
        | MutationPayload::Label { .. }
        | MutationPayload::Notification { .. }
        | MutationPayload::Conversation { .. } => None,
    }
}
...
mod tests {
    ...
    fn test_actor() -> ActorRef {
        ActorRef::Authenticated {
            actor_id: ActorId::Username(hydra_common::api::v1::users::Username::from("test-actor")),
        }
    }
    ...
    async fn walk_up_policy_issue_created_includes_self_and_parents() {
        ...
        assert!(recipients.contains(&ActorId::Username(
            hydra_common::api::v1::users::Username::from("charlie")
        )));
        ...
        assert!(recipients.contains(&ActorId::Username(
            hydra_common::api::v1::users::Username::from("bob")
        )));
    }
    ...
    async fn walk_up_policy_document_returns_empty() {
        ...
        let event = ServerEvent::DocumentCreated {
            seq: 1,
            document_id: hydra_common::DocumentId::new(),
            version: 1,
            timestamp: Utc::now(),
            payload,
        };
    ...
    fn summary_session_created() {
        ...
        let session_id: hydra_common::SessionId = "s-abcdef".parse().unwrap();
        let event = ServerEvent::SessionCreated {
            seq: 1,
            session_id,
            version: 1,
    ...
    fn event_helpers_extract_correct_info() {
        ...
        assert_eq!(event_object_id(&event), HydraId::from(issue_id.clone()));
