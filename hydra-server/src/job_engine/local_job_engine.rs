...
use super::{JobEngine, JobEngineError, JobStatus, HydraJob, SessionId};
...
pub struct LocalJobEngine {
    server_url: String,
    /// Maps hydra_id -> process info for tracking.
...
impl LocalJobEngine {
    ...
    fn build_env_vars(
        ...
        env.insert(ENV_HYDRA_ID.to_string(), hydra_id.to_string());
        env.insert(ENV_HYDRA_TOKEN.to_string(), auth_token.to_string());

        let server_url = self.server_url.trim();
        if !server_url.is_empty() {
            env.insert(ENV_HYDRA_SERVER_URL.to_string(), server_url.to_string());
        }

        // Ensure the hydra CLI is on PATH for child processes. In single-player
        // mode the server binary IS the CLI binary, so prepending its directory
        // guarantees agents can find `hydra` regardless of the user's shell setup.
    ...
    fn log_file_path(&self, hydra_id: &SessionId) -> std::path::PathBuf {
        self.log_dir.join(format!("{hydra_id}.log"))
    }

    async fn build_hydra_job(&self, hydra_id: &SessionId) -> Result<HydraJob, JobEngineError> {
        let info = self
            .processes
            .get(hydra_id)
...
impl JobEngine for LocalJobEngine {
    async fn create_job(
        ...
        let log_path = self.log_file_path(hydra_id);
        ...
        let env = self.build_env_vars(hydra_id, auth_token, env_vars);
        ...
        info!(hydra_id = %hydra_id, pid = ?pid, "local subprocess spawned");

        // Spawn a background task to wait on the child process and update status.
        let task_id = hydra_id.clone();
        tokio::spawn(async move {
    ...
    async fn list_jobs(&self) -> Result<Vec<HydraJob>, JobEngineError> {
        ...
        let keys: Vec<SessionId> = self.processes.iter().map(|e| e.key().clone()).collect();
        for key in keys {
            match self.build_hydra_job(&key).await {
                Ok(job) => jobs.push(job),
                Err(e) => {
                    warn!(hydra_id = %key, error = %e, "skipping process in list");
                }
            }
        }
    ...
    async fn find_job_by_hydra_id(&self, hydra_id: &SessionId) -> Result<HydraJob, JobEngineError> {
        self.build_hydra_job(hydra_id).await
    }
...
mod tests {
    ...
    fn insert_process(
        ...
        engine.processes.insert(
            ...
            ProcessInfo {
                ...
                completion_time,
                log_file: engine.log_file_path(hydra_id),
                status_rx: rx,
                pid,
            },
        );
    ...
    #[test]
    fn build_env_vars_includes_hydra_vars() {
        let engine = make_engine();
        let hydra_id = SessionId::new();
        let extra = HashMap::from([("CUSTOM".to_string(), "value".to_string())]);
        let env = engine.build_env_vars(&hydra_id, "test-token", &extra);

        assert_eq!(env.get(ENV_HYDRA_ID).unwrap(), &hydra_id.to_string());
        assert_eq!(env.get(ENV_HYDRA_TOKEN).unwrap(), "test-token");
        assert_eq!(
            env.get(ENV_HYDRA_SERVER_URL).unwrap(),
            "http://localhost:8080"
        );
    ...
    fn build_env_vars_omits_empty_server_url() {
        ...
        let hydra_id = SessionId::new();
        let env = engine.build_env_vars(&hydra_id, "tok", &HashMap::new());

        assert!(!env.contains_key(ENV_HYDRA_SERVER_URL));
    }
    ...
    fn build_env_vars_uses_extra_env_path_as_base() {
        ...
        let env = engine.build_env_vars(&hydra_id, "tok", &extra);
    ...
    #[tokio::test]
    async fn build_hydra_job_maps_running_status() {
        let engine = make_engine();
        let hydra_id = SessionId::new();
        insert_process(&engine, &hydra_id, ProcessStatus::Running, Some(123));

        let job = engine.build_hydra_job(&hydra_id).await.unwrap();
    ...
    #[tokio::test]
    async fn build_hydra_job_maps_complete_status() {
        let engine = make_engine();
        let hydra_id = SessionId::new();
        insert_process(&engine, &hydra_id, ProcessStatus::Complete, Some(123));
    ...
    #[tokio::test]
    async fn build_hydra_job_maps_failed_status() {
        let engine = make_engine();
        let hydra_id = SessionId::new();
        insert_process(&engine, &hydra_id, ProcessStatus::Failed, Some(123));
    ...
    #[tokio::test]
    async fn build_hydra_job_returns_not_found_for_unknown_id() {
        let engine = make_engine();
        let hydra_id = SessionId::new();

        let result = engine.build_hydra_job(&hydra_id).await;
    ...
    async fn completion_time_is_stable_across_queries() {
        ...
        let job1 = engine.build_hydra_job(&hydra_id).await.unwrap();
        let job2 = engine.build_hydra_job(&hydra_id).await.unwrap();
    ...
    async fn wait_for_exit(engine: &LocalJobEngine, hydra_id: &SessionId) {
    ...
    async fn integration_subprocess_failure_transitions_to_failed() {
        ...
        wait_for_exit(&engine, &hydra_id).await;
    ...
    async fn integration_get_logs_returns_content_after_exit() {
        ...
        let logs = engine.get_logs(&hydra_id, None).await.unwrap();
        assert!(
    ...
    async fn integration_get_logs_respects_tail_lines() {
        ...
        let tail_1 = engine.get_logs(&hydra_id, Some(1)).await.unwrap();
        assert_eq!(
            tail_1.lines().count(),
            1,
    ...
    async fn integration_get_logs_stream_returns_content() {
        ...
        let mut rx = engine.get_logs_stream(&hydra_id, false).unwrap();
