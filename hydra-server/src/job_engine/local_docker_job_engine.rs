        ...
        let inspect = self
            ...
            .map_err(|e| match &e {
                bollard::errors::Error::DockerResponseServerError {
                    status_code: 404, ..
                } => JobEngineError::NotFound(hydra_id.clone()),
...
impl JobEngine for LocalDockerJobEngine {
    ...
    async fn list_jobs(&self) -> Result<Vec<HydraJob>, JobEngineError> {
        ...
        for entry in self.containers.iter() {
            match self.inspect_to_hydra_job(entry.key()).await {
                Ok(job) => jobs.push(job),
                Err(e) => {
                    warn!(hydra_id = %entry.key(), error = %e, "skipping container in list");
                }
            }
        }
    ...
    async fn find_job_by_hydra_id(&self, hydra_id: &SessionId) -> Result<HydraJob, JobEngineError> {
        self.inspect_to_hydra_job(hydra_id).await
    }
...
mod tests {
    ...
    #[test]
    fn container_name_uses_hydra_id() {
