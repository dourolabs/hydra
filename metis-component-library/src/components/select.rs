...
pub fn Select<T: Clone + PartialEq + 'static>(props: SelectProps<T>) -> Element {
    rsx! {
        document::Link { rel: "stylesheet", href: asset!("./select.scss") }
        select::Select {
            class: "metis-select",
            value: props.value,
...
pub fn SelectTrigger(props: SelectTriggerProps) -> Element {
    rsx! {
        select::SelectTrigger { class: "metis-select-trigger", attributes: props.attributes,
            {props.children}
            svg {
                class: "metis-select-expand-icon",
                view_box: "0 0 24 24",
...
pub fn SelectGroupLabel(props: SelectGroupLabelProps) -> Element {
    rsx! {
        select::SelectGroupLabel {
            class: "metis-select-group-label",
            id: props.id,
            attributes: props.attributes,
            {props.children}
        }
