...
use hydra_bff::{BffConfig, BffState, FrontendAssets, HttpUpstream};
use tracing::info;
...
async fn main() {
    ...
    info!(upstream_url = %upstream_url, port = port, "starting hydra-bff-server");

    let state = BffState::new(upstream, config, None);
    let router = hydra_bff::build_bff_router(state);
