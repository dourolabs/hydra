...
| `/projects/engineering-v2/statuses/in-review.md` | `tests/e2e/fixtures/projects/engineering-v2/statuses/in-review.md` | **`reviewer` delta** — read the patch, decide a verdict, and submit the attached `/forms/review.yaml`: `request_changes` (transitions back to `in-development` and posts the form's `review_comment` field as an issue comment via `Effect::UpdateIssue { add_comment_from: Some("review_comment") }`) or `approve` (transitions to `pending-release`); do NOT file a child review-request — the form action drives both verdict and status transition. |
...
6. Wait for `reviewer` to submit the review form with `request_changes`. Confirm:
   ...
   - A new issue comment is posted containing the form's `review_comment` field (via `Effect::UpdateIssue { add_comment_from: Some("review_comment") }` per design [[d-druoexk]] §4 "Spawn dispatch and on_enter automation").
