...
4. Within the next dispatcher tick (≤15s), assert all of:
   - **CLI:** `hydra conversations list --status active` returns a row whose `conversation_id` is associated with `${ISSUE_ID}`. (The `--spawned-from` flag is not yet wired on the CLI per [[i-xipntwnu]] PR 2; until it is, filter client-side with `jq` on the JSONL output: `hydra --output-format jsonl conversations list --status active | jq -r 'select(.spawned_from == "'"${ISSUE_ID}"'") | .conversation_id'`.) Capture the id as `${CONV_ID}`. Confirm `spawned_from == ${ISSUE_ID}`.
   - **CLI:** `hydra conversations get ${CONV_ID}` returns the same `spawned_from` value and a non-null `creator` matching the spawn-side actor (`agents/swe` per the project's `pair-development.on_enter`).
...
5. Confirm `greet_user: true` and autonomous start:
   - **CLI:** `hydra sessions list --conversation ${CONV_ID}` returns at least one session row. Capture its `session_id` as `${SESSION_ID}`. With `--output-format jsonl` the row should carry `mode.type == "interactive"` and `mode.greet_user == true` (per [[d-ulhrefm]] §3 "Spawn behavior" — `SpawnConversationSessionsAutomation` mints `greet_user = spawned_from.is_some()`; the `SessionMode` enum is serialized with `#[serde(tag = "type", rename_all = "snake_case")]` so the JSON shape is `{ "type": "interactive", "conversation_id": ..., "greet_user": true }`).
   ...
   - **CLI:** `hydra --output-format jsonl sessions list --conversation ${CONV_ID} | jq -r '.env_vars.HYDRA_ISSUE_ID'` returns `${ISSUE_ID}` (per [[d-ulhrefm]] §3 — interactive sessions carry the same `HYDRA_ISSUE_ID` env var as headless ones, so SWE's standard CLI flow keeps working).
...
1. Continuing from Step 2 on the same `${ISSUE_ID}` / `${CONV_ID}`, wait for the in-conversation `swe` agent to:
   - Produce at least one patch on `dourolabs/hydra-test-fixture` (visible on `/patches?issue=${ISSUE_ID}` and via `hydra patches list --issue ${ISSUE_ID}`); and
...
4. Invariants:
   ...
   - `hydra patches list --issue ${ISSUE_ID}` shows at least one patch with a non-empty diff against `dourolabs/hydra-test-fixture`.
   - `hydra sessions list --from ${ISSUE_ID}` shows the in-conversation `swe` session (`mode.type == "interactive"`, `mode.greet_user == true`, attached to `${CONV_ID}`) **plus** a `reviewer` session spawned after the `in-review` transition (`mode.type == "headless"`, no conversation attachment). The two-mode mix on a single issue is the uniformity guarantee from [[d-ulhrefm]] §1 — patch and review semantics are mode-agnostic.
...
3. Within the dispatcher tick, capture the spawned conversation id as `${CONV_ID_2}` (same `hydra conversations list` + `jq` filter as Step 2.4). Confirm `${CONV_ID_2}` is `Active` and the dashboard issue header shows the "Open Conversation" affordance.
...
- **`mode.greet_user` is `false` on the spawned session** → PR 3 [[i-bhjilhhs]] regression (the three-line `SpawnConversationSessionsAutomation` change). Confirm via `hydra --output-format jsonl sessions list --conversation ${CONV_ID}`.
