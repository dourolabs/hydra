...
- When filing child issues via `hydra issues create`, set their `project_id` to `engineering-v2` (`--project engineering-v2`) so they pick up this project's pipeline. Pass `--status inbox` to file them on the project's landing column.
